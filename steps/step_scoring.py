"""
step_scoring.py — Scoring engine
=================================
Thay đổi từ bản cũ:

1. Dùng finance_score_* fields từ cache (precomputed trong step_finance_scan)
   → CF group và Growth group giờ có data thực
...
def score_symbol(row: dict, context: dict, news_scores: dict) -> dict:
    ...
    if price and ema20:
        if price > ema20: add("trend",  5, "Price>EMA20")
        else:             add("trend", -5, "Price<EMA20")

    adx = row.get("adx")
    if adx:
        if adx > 25: add("trend", 5, f"ADX={adx} strong")
        elif adx < 20: add("trend", 0, f"ADX={adx} sideways")

    supertrend = row.get("supertrend")
    if supertrend and price:
        if price > supertrend: add("trend",  5, f"ST={supertrend} bullish")
        else:                  add("trend", -5, f"ST={supertrend} bearish")
    ...
    cmf = row.get("cmf")
    if cmf is not None:
        if cmf > 0.1:   add("volume",  10, f"CMF={cmf} inflow")
        elif cmf < -0.1: add("volume", -10, f"CMF={cmf} outflow")

    mfi = row.get("mfi")
    if mfi is not None:
        if mfi < 20:   add("volume",  10, f"MFI={mfi} oversold")
    ...
    if bb_pos is not None:
        if bb_pos < 0.2:   add("volume",  5, f"BB pos={bb_pos} near lower")
        elif bb_pos > 0.8: add("volume", -5, f"BB pos={bb_pos} near upper")
    ...
    if ff_net_5d  is not None: add("ff",  5 if ff_net_5d  > 0 else -5, "FF net buy 5d" if ff_net_5d > 0 else "FF net sell 5d")
    if ff_net_20d is not None: add("ff",  5 if ff_net_20d > 0 else -5, "FF net buy 20d" if ff_net_20d > 0 else "FF net sell 20d")
    if ff_trend   is not None: add("ff",  5 if ff_trend   > 0 else -5, "FF trend accumulating" if ff_trend > 0 else "FF trend distributing")
    ...
    if cfo is not None:
        if cfo > 0: add("cf",   5, "CFO>0 real cash")
        else:       add("cf", -10, "CFO<0 cash burn")
    ...
    if rev_g is not None:
        if rev_g > 0.20:    add("growth",  5, f"RevGrowth={rev_g:.1%} strong")
        elif rev_g > 0.10:  add("growth",  3, f"RevGrowth={rev_g:.1%} good")
        elif rev_g > 0:     add("growth",  1, f"RevGrowth={rev_g:.1%} positive")
        elif rev_g < -0.10: add("growth", -3, f"RevGrowth={rev_g:.1%} declining")
        else:               add("growth", -1, f"RevGrowth={rev_g:.1%} slight decline")

    if np_g is not None:
        if np_g > 0.20:    add("growth",  5, f"ProfitGrowth={np_g:.1%} strong")
        elif np_g > 0.10:  add("growth",  3, f"ProfitGrowth={np_g:.1%} good")
        elif np_g > 0:     add("growth",  1, f"ProfitGrowth={np_g:.1%} positive")
        elif np_g < -0.10: add("growth", -3, f"ProfitGrowth={np_g:.1%} declining")
        else:               add("growth", -1, f"ProfitGrowth={np_g:.1%} slight decline")
...
if __name__ == "__main__":
    ...
    if today_index is None:
        log.warning("news_today_index.json not found — news_score = neutral (5.0)")
    ...
    for row in deep_raw:
        ...
        ev_summary = "; ".join(
            f"{e.get('type','?')}:{e.get('source','?')}:{e.get('title','')[:25]}"
    ...
    df_for_csv = df_signals.drop(columns=["news_evidence", "_ohlcv_5d"], errors="ignore")
