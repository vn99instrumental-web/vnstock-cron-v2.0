...
CHANGELOG:
  2026-05-25 — FIX BUG TPC `'float' object is not subscriptable`:
    - Symbol có TA failure (<20 rows history) → _ohlcv_5d không được set,
      defaults về [] khi load. OK với code cũ.
    - Nhưng nếu deep_raw bị corrupted ở fields khác (e.g. ohlcv_5d entry
      thành float vì JSON quirk), accessor `d["close"]` crash cả symbol.
    - Fix multi-layer:
      a) Defensive isinstance() checks trong build_summary
      b) Try-except wrap toàn bộ fetch_one → return error-marker dict
      c) Main loop catch và record error symbol thay vì lose entry
      → order_flow.json LUÔN có đúng 20 entries (kể cả symbol error)
"""
import os
import sys
...
def _safe_get_last_dict(seq):
    """
    Lấy phần tử cuối của seq, đảm bảo nó là dict.
    Trả None nếu seq không hợp lệ hoặc phần tử cuối không phải dict.
    Defensive vs corrupted data (e.g. JSON quirk biến dict → float).
    """
...
def _safe_iter_dicts(seq):
    """Iterate chỉ các phần tử là dict, skip silent những phần tử corrupted."""
...
def build_summary(symbol: str, df_intra: pd.DataFrame,
    ...
    # POC — guard với try/except để symbol không die nếu data weird
    try:
        poc_count_idx = df_vp["trade_count"].idxmax()
        poc_vol_idx   = df_vp["volume"].idxmax()
        # df_vp.loc[idx, col] có thể trả Series nếu idx duplicate
        # Force scalar conversion via .iloc + manual lookup
        poc_by_count_raw  = df_vp.loc[poc_count_idx, "price"]
        poc_by_volume_raw = df_vp.loc[poc_vol_idx,   "price"]

        # Defensive: convert Series → scalar nếu cần
    ...
    # Current price vs POC — defensive với _ohlcv_5d corrupt
    ...
    # Volume spike vs 5D avg — defensive iter
    vols = []
...
def _error_result(deep_row: dict, error_msg: str) -> dict:
    """Build minimal result dict khi fetch_one fails — giữ đủ 20 entries."""
...
def fetch_one(deep_row: dict, market_open: bool) -> dict:
    ...
    Wrap toàn bộ trong try-except: nếu bất kỳ exception nào xảy ra,
    return error-marker dict để giữ entry trong output (không bị mất symbol).
    """
    symbol = deep_row.get("symbol", "?")
    ...
    try:
        ...
        # Catch-all: lỗi bất kỳ → log với traceback + return error-marker
...
if __name__ == "__main__":
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        for future in as_completed(future_map):
            ...
            try:
                ...
                # Future-level error (shouldn't happen since fetch_one catches all,
                # but defensive layer for ThreadPool issues)
                log.error(f"  ❌ {sym} future error: {e}")
                results_map[sym] = _error_result(deep_row, f"future error: {e}")

    # Restore original order — LUÔN giữ đủ entries
    results = []
    error_count = 0
    for r in deep_raw:
        sym = r["symbol"]
    ...
    log.info(
        f"Saved order_flow.json — {len(results)} symbols "
