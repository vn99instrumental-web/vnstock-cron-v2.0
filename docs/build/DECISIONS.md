...
- **Cập nhật 2026-09-16 (migration 0009):** mở rộng `v4_hit_factor_corr` ra **TẤT CẢ biến số** trong breakdown (45 biến: s_* 16, trade/hold_*_norm 12, rank_* 6, ff_intra_*/of_bp 6, khác 5) + cột `grp` để UI lọc theo nhóm. **Blocklist** biến rò rỉ kết quả (`result_5d/30d`), ID/version, score shadow (`_*delta`, `score_trade_*`), mức giá/ngưỡng (`price/entry/tp/stop`). Insight mới: `ff_intra_net` −0.40 (khối ngoại mua ròng ↔ lãi 5 phiên thấp hơn), `trade_breakout_norm` corr_win −0.40. Lưu ý biến trùng bản chất (vd s_mkt_context = trade_context_norm) hiện cùng hệ số — chấp nhận, vẫn là biến lưu riêng.
- **Cập nhật 2026-09-16 (migration 0010):** thêm 2 view: `v4_factor_pair_corr` (ma trận tương quan **biến×biến**, nửa trên fa<fb, n≥50) để bắt đa cộng tuyến — heatmap + list "cặp gần trùng" (|r|≥0.85). Phát hiện thật: `s_growth_core = trade_growth_norm = hold_growth_norm` (r=1.0), `s_mkt_context = trade_context_norm` (1.0), `rank_ff_grp ≈ s_ff_net` (0.95) → nhiều biến là bản sao. Và `v4_hit_factor_corr_split` (tương quan **tách theo confidence & regime**, min n/bucket=25). Contrast mạnh: `s_rs_reversal` HIGH −0.26 vs MEDIUM +0.47 (corr_win +0.63); regime DEEP_DOWN +0.28 vs DOWNTREND −0.27. Cảnh báo: mẫu Jul-Aug lệch nặng DEEP_DOWN (231/333) & LOW conf mỏng (37) → UI gắn cờ n<40.
